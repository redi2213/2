...
def build_sentence(line_name, dispatch, cause, off_time, off_amp, on_time, on_amp, protective=None):
    """دو خط متن مربوط به یک خاموشی را می‌سازد؛ خاموشی حفاظتی بدون عبارت درخواست دیسپاچینگ."""
    lines = []

    # اگر ستون «قطع حفاظتی» مقدار داشته باشد، قطع حفاظتی محسوب می‌شود.
    is_protective = protective not in (None, "", 0, "0", "۰", False)

    if is_protective:
        off_sentence = (
            f"خط {line_name} به علت {cause} "
    ...
    for day_num in sorted(outages_by_day.keys()):
        ...
        for idx, o in enumerate(day_outages, start=1):
            sentences = build_sentence(
                ...
                o["off_time"], o["off_amp"], o["on_time"], o["on_amp"], o.get("protective"),
            )
...
def main():
    ...
    day_from = prompt_int("Enter start day (e.g. 5): ")
    day_to = prompt_int("Enter end day (e.g. 10): ")
    if day_to < day_from:
